...
def grid_to_world(row, col, resolution=0.4, z=1.2, yaw=0.0, x_offset=0.0, y_offset=0.0):
    x = col * resolution + x_offset
    y = row * resolution + y_offset
    return x, y, z, yaw


def convert_nodes_to_waypoints(robot_nodes_w_aps, grid_nodes, resolution,
                               transition_time=0, x_offset=0.0, y_offset=0.0):
...
if __name__ == '__main__':
    # 示例：偏移整个地图 2.0 米 (x 方向) 和 1.5 米 (y 方向)
    generate_full_yaml(x_len=5, y_len=5,
                       output_path='./yaml/20250506_map_w_edges.yaml')
