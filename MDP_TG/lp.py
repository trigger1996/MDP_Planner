...
def is_state_equal(x, y):
    x_name = str(x[0])
    y_name = str(y[0])
    if ('_in' not in x_name and '_out' not in x_name) and ('_in' not in y_name and '_out' not in y_name):
        return x == y
    else:
        x_dra_state = x[2]
        y_dra_state = y[2]
        ...
        if '_in' in y_name:
            y_name = y_name.replace('_in', '')
        elif '_out' in y_name:
            y_name = y_name.replace('_out', '')

        return (x_name == y_name) and (x_dra_state == y_dra_state)
...
def find_corresponding_incoming_states(x, state_set):
    ...
    if ('_in' in x_name):
        ...
        for state_t in state_set:
            if x_name_p in str(state_t[0]) and x_dra_state == state_t[2]:
                return state_t
        return None

def find_corresponding_outgoing_states(x, state_set):
    x_name = str(x[0])
    if ('_out' in x_name):
        return None
    else:
        x_name_p = x_name + '_out'
        x_dra_state = x[2]
        for state_t in state_set:
            if x_name_p in str(state_t[0]) and x_dra_state == state_t[2]:
                return state_t
...
def find_original_states(x):
    x_name = str(x[0])
    if '_in' in x_name:
        return (x_name[0].replace('_in', ''), x[1], x[2])
    elif '_out' in x_name:
        return (x_name[0].replace('_out', ''), x[1], x[2])
    else:
        return x
    ...
    for l, S_fi in enumerate(prod_mdp.Sf):                                                  # prod_mdp.Sf 对应所有的AMEC
        ...
        for k, MEC in enumerate(S_fi):
            ...
            if y_in_sf:
                ...
                #plan_suffix, suffix_cost, suffix_risk = syn_plan_suffix2(prod_mdp, MEC, y_in_sf)
...
def syn_plan_suffix2(prod_mdp, MEC, y_in_sf):
    # Different from the original code
    # this function strictly follows the paper and divide the set Se to two virtual copies
    # ----Synthesize optimal plan suffix to stay within the accepting MEC----
    # ----with minimal expected total cost of accepting cyclic paths----
    print_c("===========[plan suffix synthesis starts]", color=42)
    #
    sf = MEC[0]                     # MEC
    ip = MEC[1]                     # MEC 和 ip 的交集
    act = MEC[2].copy()             # 所有状态的动作集合全集
    ...
    for init_node in prod_mdp.graph['initial']:
        ...
        for state_t in ip:
            #
            # Objective 1: Create I_in and I_out
            state_t_in = (str(state_t[0]) + '_in', state_t[1], state_t[2])
            state_t_out = (str(state_t[0]) + '_out', state_t[1], state_t[2])
            #
            i_in.append(state_t_in)
            i_out.append(state_t_out)
            #
            act[state_t_in] = []
            act[state_t_out] = act[state_t]
        #
        for state_t in ip:
            #
            state_t_in = (str(state_t[0]) + '_in', state_t[1], state_t[2])
            state_t_out = (str(state_t[0]) + '_out', state_t[1], state_t[2])
            # for debugging
            if state_t == ('6_in', frozenset(), 0):
                print(233)
            #
            # Objective 2:  Modifiy product mdp with i_in and i_out
            # DO NOT combine the above two FOR-LOOP because the find_corresponding_incoming_states / find_corresponding_outgoing_states function may fail!
            for in_edges in list(prod_mdp.in_edges(state_t, data=True)):
                state_p = in_edges[0]
                edge_attr = in_edges[2]['prop']
                if state_p == state_t:
                    prod_mdp_p.add_edge(state_t_out, state_t_in, prop=edge_attr)
                else:
                    state_p_p = find_corresponding_outgoing_states(state_p, i_out)
                    if state_p_p != None:
                        prod_mdp_p.add_edge(state_p_p, state_t_in, prop=edge_attr)
                    else:
                        prod_mdp_p.add_edge(state_p, state_t_in, prop=edge_attr)
            #
            print_c('added incoming edges: ', color=35)
            print_c(list(prod_mdp_p.in_edges(state_t_in, data=True)), color=35)
            for out_edges in list(prod_mdp.out_edges(state_t, data=True)):
                state_p = out_edges[1]
                edge_attr = out_edges[2]['prop']
                if state_p == state_t:
                    prod_mdp_p.add_edge(state_t_out, state_t_in, prop=edge_attr)
                else:
                    state_p_p = find_corresponding_incoming_states(state_p, i_in)
                    if state_p_p != None:
                        prod_mdp_p.add_edge(state_t_out, state_p_p, prop=edge_attr)
                    else:
                        prod_mdp_p.add_edge(state_t_out, state_p, prop=edge_attr)
            #
            print_c('added outgoing edges: ', color=36)
            print_c(list(prod_mdp_p.out_edges(state_t_out, data=True)), color=36)
        #
        ...
        try:
            ...
            for k, s in enumerate(Sn):
                ...
                constr3 = 0
                constr4 = 0
                for u in act[s_out]:                #for u in act[s]:
                    #
                    # 这里也有不同
                    # prefix
                    #       s in Sr
                    ...
                    #       s in Sn
                    constr3 += Y[(s_out, u)]
                for f in prod_mdp_p.predecessors(s_in):
                    #
                    # 这里也有不同
                    # prefix
                    #       if f in Sr:
                    # suffix
                    #       if f in Sn 且 s in Sn 且 s not in ip
                    if (f in Se_p) and (s_in not in i_in):                       # if (f in Sn) and (s not in ip):
                        prop = prod_mdp_p[f][s_in]['prop'].copy()
                        for uf in act[f]:
                            if uf in list(prop.keys()):
                                constr4 += Y[(f, uf)]*prop[uf][0]
                            else:
                                constr4 += Y[(f, uf)]*0.00
                    if (f in Se_p) and (s_in in i_in) and not is_state_equal(s_in, f):              # if (f in Sn) and (s in ip) and (f != s):
                        prop = prod_mdp_p[f][s_in]['prop'].copy()
                        for uf in act[f]:
                            if uf in list(prop.keys()):
                                constr4 += Y[(f, uf)]*prop[uf][0]
